export default function UploadBar({ file, isUploading, hasUploaded, onFileChange, onUpload, onKeyDown }) {
        ...
        <div className="bg-[#181818] border-t border-[#2c2c2c] p-4 flex-shrink-0 w-full">
            ...
            >
                ...
                <span className="upload-text flex-1 text-left text-sm text-[#9ca3a1] truncate px-2">
                    {isUploading ? "Đang xử lý..." : file ? file.name : hasUploaded ? "Vui lòng tải ảnh khác." : "Tải ảnh công thức toán học để bắt đầu."}
                </span>

                <button
                    onClick={onUpload}
                    className={`bg-[#3a3a3a] rounded-xl p-2 text-[#d4d4d8] w-10 h-10 flex items-center justify-center transition-colors duration-200
                        ${!file || isUploading ? "opacity-50 cursor-not-allowed" : "hover:bg-[#4b4b4b]"}`}
                    disabled={!file || isUploading}
                >
                    {isUploading ? <i className="fas fa-spinner fa-spin"></i> : <i className="fas fa-arrow-up"></i>}
                </button>
            </div>
        </div>
    );
